...
document.addEventListener('DOMContentLoaded', function() {
    ...
    calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventClick: function(info) {
            ...
            document.getElementById('eventOptionsTitle').innerText = currentEventTitle;
...
function loadSchedule(calendar) {
    db.collection("schedule").onSnapshot((snapshot) => {
        ...
        snapshot.forEach((doc) => {
            const data = doc.data();
            calendar.addEvent({
                id: doc.id, title: data.title, start: data.date, allDay: true,
...
async function generateShoppingList() {
    ...
    if(selectHtml.options.length <= 1) listHtml.innerHTML = "<li class='list-group-item'>Scanning Schedule...</li>";
    
    // 1. Get Future Events
    const futureEvents = calendar.getEvents().filter(e => e.start >= new Date().setHours(0,0,0,0));
    ...
    futureEvents.forEach(event => {
        ...
        if (!weeksData[dateKey]) weeksData[dateKey] = { prettyDate: weekStart.toLocaleDateString('en-US', { month: 'short', day: 'numeric' }), meals: [] };
    ...
    const recipeMap = {};
    recipeSnap.forEach(doc => recipeMap[doc.data().name.toLowerCase()] = doc.data().ingredients || []);
    
    // 4. Build Dropdown
    const currentSelection = selectHtml.value;
    selectHtml.innerHTML = "";
    
    Object.keys(weeksData).sort().forEach((dateKey, index) => {
...
function renderSelectedWeek() {
    ...
    if (ingredients.length > 0) {
        ...
        ingredients.forEach(line => {
            ...
            shoppingMap[key] ? shoppingMap[key].qty += parsed.qty : shoppingMap[key] = { qty: parsed.qty, unit: parsed.unit, name: parsed.name };
        });
        
        Object.keys(shoppingMap).sort().forEach(key => {
            const item = shoppingMap[key];
            let display = item.unit === "" ? `${Math.round(item.qty*100)/100}x ${capitalize(item.name)}` : `${Math.round(item.qty*100)/100} ${item.unit} ${capitalize(item.name)}`;
    ...
    } else listHtml.innerHTML = "<li class='list-group-item text-danger'>No ingredients found!</li>";
}
...
// --- HELPERS ---
function saveMealToDate(date, mealName) { db.collection("schedule").add({ date: date, title: mealName }); }
function deleteRecipe(id, name) { if(confirm("Permanently delete '" + name + "'?")) db.collection("recipes").doc(id).delete(); }
function deleteCurrentEvent() { if (confirm("Remove meal?")) db.collection("schedule").doc(currentEventId).delete().then(()=>bootstrap.Modal.getInstance(document.getElementById('eventOptionsModal')).hide()); }
function openRecipeFromEvent() {
    bootstrap.Modal.getInstance(document.getElementById('eventOptionsModal')).hide();
    db.collection("recipes").where("name", "==", currentEventTitle).get().then((q) => { if(!q.empty) openRecipeModal(q.docs[0].id); });
}
function openRecipeModal(id) {
    db.collection("recipes").doc(id).get().then((doc) => {
        if (doc.exists) {
            document.getElementById("modalRecipeTitle").innerText = doc.data().name;
            document.getElementById("modalInstructions").innerText = doc.data().instructions || "No instructions.";
            document.getElementById("modalIngredients").innerHTML = doc.data().ingredients.map(i => `<li class="list-group-item">${i}</li>`).join('');
...
function filterRecipes() {
    ...
    document.querySelectorAll('.recipe-item').forEach(i => i.innerText.toLowerCase().includes(q) ? i.classList.replace('d-none','d-flex') : i.classList.replace('d-flex','d-none'));
}
// --- REPLACE THE copyListToClipboard FUNCTION WITH THIS ---
...
function parseLine(line) {
    line = line.replace(/^[\*\-\•]\s*/, '').trim().toLowerCase();
    const match = line.match(/^(\d+[\/\.]\d+|\d+)\s*/);
    let qty = 1; let remainder = line;
    if (match) {
        qty = match[1].includes('/') ? parseInt(match[1].split('/')[0]) / parseInt(match[1].split('/')[1]) : parseFloat(match[1]);
        remainder = line.replace(match[0], '').trim();
    }
    const parts = remainder.split(' ');
    let unit = ['cup','cups','tbsp','tsp','oz','lb','lbs','g','kg','ml','l','can','cans','clove','cloves','jar','jars','packet','packets'].includes(parts[0]) ? parts[0] : "";
